...
function resume_xspot()
{
    ...
    if [[ "$host" == "xspot-vm"* ]]; then
        ...
        pool_name="${host_fields[2]}-${host_fields[3]}"
...
END
        cat $TMP_FILE
        http_code=$(curl -s -w '\n%{http_code}' -d "@$TMP_FILE" -H 'Content-Type: application/json' -X POST http://$XCOMPUTE_HEAD_IP:5000/v1/xcompute/vm | tail -n 1)
        echo "http_code=$http_code"
        if [ $http_code -ne 200 ]; then
            echo "`date` Resume FAILED; curl = $http_code" >> $SLURM_AWS_LOG
        fi
