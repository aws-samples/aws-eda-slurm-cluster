...
function on_exit {
    ...
    if [[ $rc -ne 0 ]] && [[ ":$SOCA_ERROR_SNS_TOPIC_ARN" != ":" ]]; then
        aws sns publish --region $AWS_DEFAULT_REGION --topic-arn $SOCA_ERROR_SNS_TOPIC_ARN --subject "$INSTANCE_NAME slurmdb_config.sh failed" --message "See /var/log/cloud-init.log or grep cloud-init /var/log/messages | less for more info."
    fi
...
hostnamectl set-hostname --static $SlurmDbdHostname.$Domain
...
pushd $PLAYBOOKS_PATH
ansible-playbook $PLAYBOOKS_PATH/SlurmDbd.yml \
...
# Set up motd
figlet -f slant "SLURM DB" > /etc/motd
